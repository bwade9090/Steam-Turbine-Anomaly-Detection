...
def GAN_loss(Z, X, generator_settings, discriminator_settings):
    ...
    D_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, labels=tf.zeros_like(D_logit_real)), 1)
    D_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=tf.ones_like(D_logit_fake)), 1)
    ...
    G_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=tf.zeros_like(D_logit_fake)), 1)
    # normal is 0, anomaly is 1!!! babodra!!
...
def generator(z, hidden_units_g, seq_length, batch_size, num_signals, reuse=False, parameters=None, learn_scale=True):
    ...
    with tf.variable_scope("generator") as scope:
        ...
        # progected data's values are not [-1,1]
        output_3d = tf.reshape(logits_2d, [-1, seq_length, num_signals])
...
def discriminator(x, hidden_units_d, seq_length, batch_size, reuse=False, parameters=None, batch_mean=False):
    with tf.variable_scope("discriminator") as scope:
        ...
        if parameters is None:
            ...
            b_out_G_initializer = tf.constant_initializer(value=parameters['discriminator/b_out_D:0'])
