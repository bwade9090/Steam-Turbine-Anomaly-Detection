...
def ewp(seq_length, seq_step, num_signals):
    train = np.loadtxt(open('./data/ewp_train.csv'), delimiter=',')
    print('Loaded ewp_train.csv')
    m, n = train.shape # m=22176, n=361
    # normalization
    for i in range(n):
        ...
        if A != 0:
            ...
            train[:,i] = np.zeros(m) # Aleady I have deleted the constant columns.

    samples = train # ewp_train doesn't include labels.
    labels = np.zeros(m) # the train data is normal.
    ...
    pca = PCA(n_components, svd_solver = 'full')
    pca.fit(X_n)
    ex_var = pca.explained_variance_ratio_ # when pc=6, ex_var=[0.3181, 0.1872, 0.1391, 0.0571, 0.03730, 0.0312] ,ex_var.sum()=0.7698.
    pc = pca.components_
    # np.save('./data/ewp_pc_%d.npy'%n_components, pc)

    # project values on the principal component
    T_n = np.matmul(X_n, pc.transpose(1,0))
    samples = T_n
...
def ewp_test(seq_length, seq_step, num_signals):
    # ewp_test consists of normal data and anomaly data.
    # test_normal part
    test1 = np.loadtxt(open('./data/ewp_test_normal.csv'), delimiter=',')
    print('Loaded ewp_test_normal.csv')

    m1, n1 = test1.shape # m1=4320, n1=361

    for i in range(n1):
        B1 = max(test1[:,i]) - min(test1[:,i])
        if B1 != 0:
            test1[:,i] = (test1[:,i] - min(test1[:,i])) / (max(test1[:,i]) - min(test1[:,i]))
            # scale from -1 to 1
            test1[:,i] = 2 * test1[:,i] - 1
        else:
            test1[:,i] = np.zeros(m1) # constant columns aleady have been deleted.

    samples1 = test1 # test1 does not have labels.
    labels1 = np.zeros(m1) # test1 is normal data.
    idx1 = np.asarray(list(range(0,m1))) # record the idx of each point

    # -- apply PCA dimension reduction for multi-variate GAN-AD -- #
    X_a1 = samples1
    
    pc = np.load('./data/ewp_pc_%d.npy'%num_signals) # pc from ewp_train data
    print('pc form ewp_pc_%d.npy'%num_signals)

    T_a1 = np.matmul(X_a1, pc.transpose(1,0))
    samples1 = T_a1

    num_samples_t1 = (samples1.shape[0] - seq_length) // seq_step
    aa1 = np.empty([num_samples_t1, seq_length, num_signals])
    bb1 = np.empty([num_samples_t1, seq_length, 1])
    bbb1 = np.empty([num_samples_t1, seq_length, 1])

    for j in range(num_samples_t1):
        bb1[j, :, :] = np.reshape(labels1[(j * seq_step):(j * seq_step + seq_length)], [-1,1])
        bbb1[j, :, :] = np.reshape(idx1[(j * seq_step):(j * seq_step + seq_length)], [-1,1])
        for i in range(num_signals):
            aa1[j, :, i] = samples1[(j * seq_step):(j * seq_step + seq_length), i]
    ...
    # test_anomaly part
    test2 = np.loadtxt(open('./data/ewp_test_anomaly_before.csv'), delimiter=',')
    print('Loaded ewp_test_anomaly_before.csv')

    m2, n2 = test2.shape # m2 = 3150, n1 = 361
    
    for i in range(n2):
        B2 = max(test2[:,i]) - min(test2[:,i])
        if B2 != 0:
            test2[:,i] = (test2[:,i] - min(test2[:,i])) / (max(test2[:,i]) - min(test2[:,i]))
            # scale from -1 to 1
            test2[:,i] = 2 * test2[:,i] - 1
        else:
            test2[:,i] = np.zeros(m2) # constant columns aleady have been deleted.

    samples2 = test2
    labels2 = np.ones(m2) # test2 is anomaly data
    idx2 = np.asarray(list(range(m1, m1 + m2))) # start from the end point of test1
    #idx2 = np.asarray(list(range(0,m2)))

    # -- apply PCA dimension reduction for multi-variate GAN-AD -- #
    X_a2 = samples2

    # pc have been loaded above
    T_a2 = np.matmul(X_a2, pc.transpose(1,0))
    samples2 = T_a2

    num_samples_t2 = (samples2.shape[0] - seq_length) // seq_step
    aa2 = np.empty([num_samples_t2, seq_length, num_signals])
    bb2 = np.empty([num_samples_t2, seq_length, 1])
    bbb2 = np.empty([num_samples_t2, seq_length, 1])

    for j in range(num_samples_t2):
        bb2[j, :, :] = np.reshape(labels2[(j * seq_step):(j * seq_step + seq_length)], [-1,1])
        bbb2[j, :, :] = np.reshape(idx2[(j * seq_step):(j * seq_step + seq_length)], [-1,1])
        for i in range(num_signals):
            aa2[j, :, i] = samples2[(j * seq_step):(j * seq_step + seq_length), i]

    samples2 = aa2
    labels2 = bb2
    index2 = bbb2

    #concatenate test1 and test2
...
def get_data(data_type, seq_length, seq_step, num_signals, sub_id, eval_single, eval_an, data_options=None):
    ...
    if data_type == 'load':
        ...
        print('data_type is ewp')
        samples, labels = ewp(seq_length, seq_step, num_signals)
    elif data_type == 'ewp_test':
        samples, labels, index = ewp_test(seq_length, seq_step, num_signals)
    else:
