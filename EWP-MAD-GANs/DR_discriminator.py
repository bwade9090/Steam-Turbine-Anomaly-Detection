...
# In this version, 'insert()' have been changed for reconstructing error since 2019.3.3.
...
def generator_o(z, hidden_units_g, seq_length, batch_size, num_generated_features, reuse=False, parameters=None, learn_scale=True):
    ...
    with tf.variable_scope("generator_o") as scope:
        ...
        # projected data's values are not [-1,1]
        output_3d = tf.reshape(logits_2d, [-1, seq_length, num_generated_features])
    return output_3d
...
def discriminator_o(x, hidden_units_d, reuse=False, parameters=None):
    ...
    with tf.variable_scope("discriminator_0") as scope:
        ...
        lstm_initializer = tf.constant_initializer(value=parameters['discriminator/rnn/lstm_cell/weights:0'])
...
def invert(settings, samples, para_path, g_tolerance=None, e_tolerance=0.1,
    ...
    reconstruction_error_per_sample = tf.abs(G_samples - samples)
    reconstruction_error_per_sample = tf.reduce_mean(reconstruction_error_per_sample)
    ...
    # reconstruction_error = tf.constant(value=1) 
    # updater
    ...
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        error = sess.run(reconstruction_error_per_sample, feed_dict=fd)
        ...
        if not n_iter is None:
            ...
            if not g_tolerance is None:
                while g_n > g_tolerance:
                    _ = sess.run(solver, feed_dict=fd)
                    error, g_n = sess.run([reconstruction_error_per_sample, grad_norm], feed_dict=fd)
                    i += 1
                    print(error, g_n)
                    if i > max_iter:
                        break
        ...
        # Kxys = sess.run(Kxy, feed_dict=fd)
        # print('Kxy: ', Kxys)
        # sim_per_sample = sess.run(similarity_per_sample, feed_dict=fd)
        # print('sim_per_sample: ', sim_per_sample)
        print('Z found in', i, 'iterations with final reconstruction error of', error_per_sample)
